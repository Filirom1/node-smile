...
SmileParser.prototype.nextToken = function(){
  ...
  console.log(this._inputBuffer, this._inputPtr, ch)
  this._typeByte = ch;
  switch ((ch >> 5) & 0x7) {
  ...
  this.emit("error", "Invalid type marker byte 0x" + (ch & 0xFF).toString(16) + " for expected value token");
  return null;
}
...
SmileParser.prototype.parseBuffer = function(buffer){
  ...
  var token = null;
  while(this._inputPtr < this._inputBuffer.length){
    token = this.nextToken();
    if(!token){
      return this;
    }
    outputBufferStream.write(token);
  }
